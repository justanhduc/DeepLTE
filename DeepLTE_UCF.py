...
class DeepLLENet(Model):
    def __init__(self, config_file, **kwargs):
        super(DeepLLENet, self).__init__(config_file, **kwargs)
    ...
    def inference_encoder(self, input, W=False):
        ...
        if W:
            ...
            x = T.tile(np.arange(self.num_frames, dtype='float32'), (c*h*w, 1)).T
            u = T.tile(W, c * h * w)
            output = utils.lagrange_interpolation(x, output, u, 3)
    ...
    def get_cost(self, input, gt, **kwargs):
        ...
        if self.order == 1 or self.order == 2:
            ...
            interp = T.concatenate([self(input, i) for i in (.5, 2.5)])
        ...
        cost = nn.huberloss(inter_fr1, self.unnormalize(gt) * 255.) * lambda1 + \
               ...
               nn.vgg16_loss(inter_fr1, self.unnormalize(gt) * 255., self.vgg_weight_file) * lambda3
